...
const BREAKFAST: Recipe = {
  ...
  color: 'var(--amber)',
  sc: 'ca',
  name: 'Berry Walnut Power Oats',
  tag: 'Prep night before - 5 min',
  prepL: 'Prep ahead',
  prepC: 'var(--teal)',
  hk: 350,
  hp: '18g',
...
const DR_EMILY: Recipe = {
  ...
  color: 'var(--amber)',
  sc: 'ca',
  name: 'Lemon & Blueberry Overnight Oats',
  tag: 'Dr Emily Prpa',
  prepL: 'Prep ahead',
  prepC: 'var(--teal)',
  hk: 450,
...
describe('recipe filtering', () => {
  ...
  it('search by ingredient returns matching recipes', () => {
    const q = 'oats'
    const matches = ALL_FIXTURES.filter(
      r =>
        r.name.toLowerCase().includes(q) || r.ings.some(([ing]) => ing.toLowerCase().includes(q)),
    )
