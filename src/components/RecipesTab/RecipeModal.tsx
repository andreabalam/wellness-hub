...
export default function RecipeModal({
  ...
  const [prepTime, setPrepTime] = useState(
    () => initialRecipe?.prepTime ?? initialRecipe?.prepL ?? '',
  )
  const [healthTag, setHealthTag] = useState<'healthy' | 'indulgent' | ''>(
    () => initialRecipe?.healthTag ?? '',
  )
      ...
      >
        ...
        <FieldRow label="Recipe name *">
          <input
            ...
            placeholder="e.g. Mango Chia Pudding"
          />
        </FieldRow>
        ...
        <FieldRow label="Category (pick one or create your own)">
          <div className="flex flex-wrap gap-6 mb-8">
            {allTags.map(tag => (
              <button
                key={tag}
                onClick={() => setCat(tag)}
                style={chipStyle(cat === tag, 'var(--purple)')}
              >
          ...
          <div className="flex gap-6">
            <input
              ...
              placeholder="New tag (e.g. Sauce, Side...)"
            />
        ...
        <FieldRow label="Prep time">
          <div className="flex flex-wrap gap-6 mb-6">
            {PREP_TIME_PRESETS.map(t => (
              <button
                key={t}
                onClick={() => setPrepTime(prepTime === t ? '' : t)}
                style={chipStyle(prepTime === t, 'var(--teal)')}
              >
                {t}
              </button>
            ))}
        ...
        <FieldRow label="Health tag">
          <div className="flex gap-8">
            <button
              onClick={() => setHealthTag(healthTag === 'healthy' ? '' : 'healthy')}
              style={chipStyle(healthTag === 'healthy', 'var(--green)')}
            >
              ✦ Healthy
            </button>
            <button
              onClick={() => setHealthTag(healthTag === 'indulgent' ? '' : 'indulgent')}
              style={chipStyle(healthTag === 'indulgent', 'var(--purple)')}
            >
              ✧ Indulgent
            </button>
          </div>
            ...
            <div className="macro-calc">
              <div className="macro-calc__controls">
                ...
                <button
                  ...
                  title={
                    ings.length
                      ? 'Look up each ingredient in the USDA food database'
                      : 'Add ingredients below first'
                  }
                >
                  {calcState === 'loading' ? (
                    <>
                      <span className="import-spinner import-spinner--sm" aria-hidden />
                      Calculating…
                    </>
                  ) : (
                ...
                <div className="macro-calc__rows">
                  {calcRows.map((r, i) => (
                    <div key={i} className={`macro-calc__row macro-calc__row--${r.status}`}>
                      ...
                      {r.status === 'no-amount' && (
                        <>
                          ⚠ {r.ing} · {r.amount} — amount unclear, skipped
                        </>
                      )}
                  ...
                  {calcState === 'done' && (
                    <div className="macro-calc__note">
                      Estimated from USDA data — review before saving.
                    </div>
                  )}
                </div>
              )}
        ...
        <FieldRow label="Ingredients">
          {ings.map(([n, a], i) => (
            <div key={i} className="ing-row">
              ...
              <button
                onClick={() => {
                  setIngs(prev => prev.filter((_, j) => j !== i))
                  resetCalc()
                }}
          ...
          <div className="flex gap-6">
            ...
            <input
              ...
              style={{ flex: 1 }}
            />
            <button onClick={addIng} className="icon-add-btn">
              +
            </button>
          </div>
        </FieldRow>
        ...
        <FieldRow label="Steps">
          ...
          <div className="flex gap-6">
            <input
              className="tinput flex-1"
              value={stepTxt}
              onChange={e => setStepTxt(e.target.value)}
              placeholder="Add a step..."
              onKeyDown={e => e.key === 'Enter' && addStep()}
            />
            <button onClick={addStep} className="icon-add-btn">
              +
            </button>
          </div>
        </FieldRow>
        ...
        {msg && (
          <div className={`mt-10 text-base text-center ${msgOk ? 'text-green' : 'text-coral'}`}>
            {msg}
          </div>
        )}
      </div>
