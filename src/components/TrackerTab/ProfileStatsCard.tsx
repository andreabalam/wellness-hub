...
function EditForm({ user, onClose, onSaved }: EditFormProps) {
  ...
  const set =
    <K extends keyof FormFields>(k: K) =>
  ...
  const fillFromOura = async () => {
    setFetching(true)
    try {
      const [info, tdee] = await Promise.all([fetchOuraPersonalInfo(), fetchOuraTdeeAvg(7)])
      setF(prev => ({
        ...prev,
  ...
  return (
    <div className="flex flex-col gap-16">
      ...
      <div>
        ...
        <div className="profile-grid">
          <label className="profile-form-lbl">
            <span className="profile-form-lspan">Weight (kg)</span>
            <input
              className="tnum"
              type="number"
              step="0.1"
          ...
          <label className="profile-form-lbl">
            <span className="profile-form-lspan">Height (m)</span>
            <input
              className="tnum"
              type="number"
              step="0.01"
          ...
          <label className="profile-form-lbl">
            <span className="profile-form-lspan">Age</span>
            <input
              className="tnum"
              type="number"
              value={f.age}
          ...
          <label className="profile-form-lbl">
            <span className="profile-form-lspan">Sex</span>
          ...
          <label className="profile-form-lbl">
            <span className="profile-form-lspan">Body fat %</span>
            <input
              className="tnum"
              type="number"
              step="0.1"
              ...
              onClick={() =>
                setF(prev => {
                  ...
                  return {
                    ...
                    unit: 'cm',
                    waist: prev.waist
                      ? String(+(parseFloat(prev.waist) * CM_PER_IN).toFixed(1))
                      : '',
                    glutes: prev.glutes
                      ? String(+(parseFloat(prev.glutes) * CM_PER_IN).toFixed(1))
                      : '',
                  }
                })
              }
      ...
      <div>
        ...
        <div className="profile-grid">
          <label className="profile-form-lbl">
            <span className="profile-form-lspan">Chronotype</span>
          ...
          <label className="profile-form-lbl">
            <span className="profile-form-lspan">Equipment</span>
            <input
              className="tinput"
              type="text"
              value={f.equipment}
          ...
          {f.sex !== 'male' && (
            <label className="profile-form-lbl">
              <span className="profile-form-lspan">Cycle type</span>
      ...
      <div>
        ...
        <div className="profile-grid">
          <label className="profile-form-lbl">
            <span className="profile-form-lspan">Fat loss rate</span>
            <select
              className="tinput"
              value={f.lossRate}
              onChange={e => setF(prev => ({ ...prev, lossRate: e.target.value, kcalOvr: '' }))}
            >
          ...
          <label className="profile-form-lbl">
            <span className="profile-form-lspan">Macro split</span>
        ...
        <div className="profile-grid--2col">
          <label className="profile-form-lbl">
            ...
            <input
              ...
              type="number"
              value={f.tdee}
              onChange={e => setF(prev => ({ ...prev, tdee: e.target.value, kcalOvr: '' }))}
              placeholder="2 100"
            />
          </label>
          <label className="profile-form-lbl">
            <span className="profile-form-lspan">
              {autoKcal > 0
                ? `Kcal target (auto: ${autoKcal.toLocaleString()})`
                : 'Kcal target override'}
            </span>
            <input
              className="tnum"
              type="number"
...
export default function ProfileStatsCard({ user, defaultOpen = false, onSaved }: Props) {
  ...
  const fatMass =
    s.weightKg > 0 && s.bodyFatPct > 0 ? +((s.weightKg * s.bodyFatPct) / 100).toFixed(1) : null
  ...
  const STATS = hasData
    ? (
        [
          ...
          (s.glutesCm ?? 0) > 0 && {
            l: 'Glutes',
            v: displayMeasure(s.glutesCm!),
            c: 'var(--teal)',
          },
          s.tdeeKcal > 0 && {
            l: 'TDEE',
            v: `~${s.tdeeKcal.toLocaleString()} kcal`,
            c: 'var(--amber-light)',
          },
          s.kcalTarget > 0 && {
            l: 'Daily target',
            v: `~${s.kcalTarget.toLocaleString()} kcal`,
            c: 'var(--green)',
          },
      ...
      ).filter((x): x is { l: string; v: string; c: string } => !!x)
    : []

  return (
    <div className="tcard mt-20 mb-20">
      <div className="flex-between" style={{ marginBottom: open ? 16 : hasData ? 12 : 0 }}>
        <div className="tlabel text-muted2" style={{ marginBottom: 0 }}>
          Profile &amp; Targets
        </div>
        <button
          onClick={() => setOpen(v => !v)}
          className={`open-toggle-btn ${open ? 'text-muted2' : 'text-teal'}`}
        >
        ...
        <div className="stats-grid">
          {STATS.map(stat => (
            <div key={stat.l} className="stat-tile">
              ...
              <div className="stat-tile__value" style={{ color: stat.c }}>
                {stat.v}
              </div>
            </div>
          ))}
      ...
      {open && <EditForm user={user} onClose={() => setOpen(false)} onSaved={onSaved} />}
    </div>
  )
}
