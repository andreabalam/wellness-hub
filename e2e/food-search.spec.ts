...
test.describe('Tracker food search', () => {
  ...
  const dropdown = (page: import('@playwright/test').Page) => page.locator('.autocomplete-dropdown')
  ...
  test('unknown food → search online → USDA result fills the form', async ({ page }) => {
    await page.route(/fdc\/v1\/foods\/search/, route =>
      route.fulfill({
        status: 200,
        contentType: 'application/json',
        body: JSON.stringify(USDA_RESPONSE),
      }),
    )

    await mealInput(page).fill('zztamales')
    await dropdown(page)
      .getByText(/Search online for/)
  ...
  test('no USDA match → AI estimate fills the form with a note', async ({ page }) => {
    ...
    await page.route('**/functions/v1/estimate-food-macros', route =>
      route.fulfill({
        status: 200,
        contentType: 'application/json',
